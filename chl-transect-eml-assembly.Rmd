...
chl_qual$iode_quality_flag <- NA_integer_

chl_qual <- chl_qual %>%
   mutate(iode_quality_flag = case_when(method_contributor == "method_Menden-Deuer" & !is.na(chl) ~ 1,
                                        method_contributor == "method_Menden-Deuer" & is.na(chl) ~ 4,
...
# maximum value of the entire data set is EN608 >0&<200 at L1 (chl = 7.742 ug/L) and is notably higher than other replicates from this Niskin (chl = 6.041 and 6.385 ug/L) and the next highest value in the data set (6.824 ug/L)
...
make_eml(path = getwd(),
         ...
         data.table.description = "Chlorophyll from discrete water column bottles cleaned for EDI",
...
# Manually inserting additionalInfo node into EML XML because additional_info.txt is deleted when running script
