...
chl_qual <- chl_qual %>%
  ...
  mutate(quality_flag = as.character(quality_flag))  %>%
...
chl_qual <- chl_qual %>%
  mutate(quality_flag = case_when(quality_flag==2 ~ "3",
                                  quality_flag==3 ~ "4",
                                  TRUE ~ quality_flag)) 
#%>%
  #mutate(quality_flag = case_when(method_contributor=="method_Menden-Deuer" & chl==!is.na
# ADD MENDEN-DEUER QUALITY FLAG MANIPULATION

# sort by cruise, cast, niskin, filter size
size_sort <- c(">0",">5",">0&<10",">10",">20")  #Need M-D? >0&<200 >5&<200 >20&<200
