...
size_sort <- c(">0",">0&<10",">5", ">20",">0&<200", ">0&<200", ">5&<200", ">20&<200")
...
qa_chl <- chl_qual %>%
  group_by(cruise, cast, filter_size) %>%
  summarise(avg_chl = mean(chl, na.rm = TRUE), 
...
#ggplot(chl_qual, aes(x=longitude, y=latitude, color=chl)) + geom_point() + scale_color_gradient(low = "green2", high = "darkgreen")
#ggplot(chl_qual, aes(x=longitude, y=latitude, color=cruise))+ geom_point()

#ggplot(subset(irmdata, YEAR == "2019"),aes(x=SALINITY_PSS78, y=DIC_UMOL_KG, color=tadate_TA_UMOL_L))+ geom_point()
