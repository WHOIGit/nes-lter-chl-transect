...
```{r}
## TODO: keep cast and niskin to later join with SMD data
## is this step needed at all? 

## API transect chl
api_chl_final <- smd_key %>% ungroup() %>% 
  select(cruise, cast_expand, niskin_expand) %>%
  left_join(api_chl, .,
            by = c("cruise", "cast" = "cast_expand", "niskin" = "niskin_expand")) 
...
#api_chl_final$cast <- as.integer(api_chl_final$cast)
#api_chl_final$niskin <- as.integer(api_chl_final$niskin)
...
# check for missing method_contributor
...
#smd_chl_final$cast <- as.integer(smd_chl_final$cast)
#smd_chl_final$niskin <- as.integer(smd_chl_final$niskin)
...
#remove original quality flag
 chl_qual <- select(chl_qual, -project_id_basic, -project_id_fancy, -alternate_sample_id)
