...
smd_expanded$cast_expand <- as.character(smd_expanded$cast_expand)
...
#smd_key$cast <- as.integer(smd_key$cast)
...
smd_key$cast <- as.character(smd_key$cast)
...
need_metadata$iode_quality_flag <- NA_integer_
...
# sort by datetime # puts rows without bottle metadata at end of file     
#chl_qual <- chl_qual[order(chl_qual$date), ]
...
# library(data.table)
# setDT(chl_dis)             # convert to data.table without copy
# setDT(need_metadata)         # convert to data.table without copy
# 
# # join and update "df" by reference, i.e. without copy 
# chl_dis[need_metadata, on = c("cruise", "cast", "niskin", "replicate"), date := i.date]
...
library(compareDF)
chl_previous <- read_csv('nes-lter-chl-transect-rosette_20230329.csv', show_col_types = FALSE) 
diff_check <- compare_df(chl_previous, chl_qual)
...
qa_smd <- smd_key %>%
  right_join(smd_chl, by = c("cruise", "cast", "niskin")) %>%
  group_by(cruise, cast_expand, bottle_other_method, filter_size) %>%
