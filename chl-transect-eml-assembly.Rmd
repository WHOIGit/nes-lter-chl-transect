...
cruiselist <- c("ar22", "ar24a", "ar24b", "ar24c", "en608", "ar28a", "ar28b","en617", "ar31a",  "ar31b", "ar31c", "ar32", "en627", "ar34a", "ar34b", "en644", "ar38", "ar39a", "ar39b", "en649", "en655", "en657", "en661", "en668", "ar61b", "at46", "en687")
#ar66b NOT included here. Sosik lab is not providing for v2. 
#ar66b IS included in calls to API for ctd and bottle data to populate SMD rows. 
...
smd_chl <- read_csv("NES-LTER-chla-T0-SMD.csv") 
# TODO: decide how to deal with niskin_second_cast. For now, remove. 
smd_chl <- smd_chl %>% 
  select(-niskin_second_cast)  %>% 
...
cruiselist <- c("ar22", "ar24a", "ar24b", "ar24c", "en608", "ar28a", "ar28b","en617", "ar31a",  "ar31b", "ar31c", "ar32", "en627", "ar34a", "ar34b", "en644", "ar38", "ar39a", "ar39b", "en649", "en655", "en657", "en661", "en668", "ar61b", "at46", "ar66b", "en687")
#ar66b NOT in call to API for chlorophyll data. Sosik lab is not providing for v2. 
#ar66b IS included in calls to API for ctd and bottle data to populate SMD rows. 
...
# add dummy quality_flag to enable column matching with API
smd_chl_final$quality_flag <- NA_integer_
...
headers <- c("cruise", "cast", "niskin", "bottle_other_method", "date", "latitude", "longitude", "depth", "method_contributor", "replicate","vol_filtered", "filter_size", "tau_calibration", "fd_calibration", "rb", "ra", "blank", "rb_blank", "ra_blank", "chl", "phaeo", "quality_flag", "iode_quality_flag")
# reorder columns as necessary
api_chl_final <- api_chl_final[, headers]
smd_chl_final <- smd_chl_final[, headers]

# merge datasets by row
chl_transect <- bind_rows(api_chl_final, smd_chl_final) 
...
chl_transect <- chl_transect  %>% 
    mutate(project_id_fancy  = case_when(cruise == "AR66B" & method_contributor == "method_Menden-Deuer" ~ "LTER",
                                   cruise == "AR22" ~ "JP",
                                   cruise == "AR32" ~ "JP",
                                   cruise == "AR38" ~ "JP",
                                   cruise == "AT46" ~ "LTER"))
...
# # add column for ooi join below
# chl_transect$alternate_sample_id <- NA_character_

```
# Check the flag and method_contributor column

```{r}

# check that there are no NAs in quality_flag for method_contributor Sosik or Rynearson ie no missing joins
qual_missing <- chl_transect %>%
  filter(method_contributor == "method_Sosik" | method_contributor == "method_Rynearson") %>%
  filter(is.na(quality_flag))
# write csv for providers to evaluate
#write.csv(qual_missing, "chl-transect-missing-quality.csv", row.names = FALSE)

qual_iode_missing <- chl_transect %>%
...
qual_incorrect <- chl_transect %>%
  filter(method_contributor == "method_Menden-Deuer") %>%
  filter(!is.na(quality_flag))

qual_iode_incorrect <- chl_transect %>%
  filter(method_contributor == "method_Sosik" | method_contributor == "method_Rynearson") %>%
  filter(!is.na(iode_quality_flag))
...
# chl_ar24$quality_flag <- NA_integer_
...
# 
# # define headers for columns in desired order
# headers_join <- c("cruise", "cast", "niskin", "bottle_other_method", "date", "latitude", "longitude", "depth", "method_contributor", "replicate", "alternate_sample_id", "filter_size",  "chl", "phaeo","quality_flag", "iode_quality_flag", "project_id")
# # reorder columns as necessary
...
# This section no longer needed as flags now come from the API. 
...
#v2 has no AR24A cast 2 samples; removed per ETC communication they are really cast 3 and have been corrected in API 

ar24missing <-read_csv('samples_lacking_bottle_metadata-v2.csv')
...
ar24missing$date <- as.POSIXct(ar24missing$date, tz = "GMT", "%m/%d/%Y %H:%M")

ar24missing$iode_quality_flag <- NA_integer_

chl_qual <- bind_rows(chl_dis, ar24missing)
...
#remove original quality flag
 chl_qual <- select(chl_qual, -quality_flag, -project_id_basic, -project_id_fancy, -alternate_sample_id)
...
# Insert Custom Project Node
project_insert(edi_pkg = pkg_id, "parent_project_RAPID_OOI.txt")

#project_insert(edi_pkg = pkg_id, "parent_project.txt")

#ediutilities::project_insert(edi_pkg = pkg_id, "parent_project_RAPID.txt")
