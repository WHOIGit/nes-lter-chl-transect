...
size_sort <- c(">0",">5","<10",">10",">20")
cast_sort <- str_sort(unique(chl_transect$cast), numeric = TRUE)
niskin_sort <- str_sort(unique(chl_transect$niskin), numeric = TRUE)

chl_transect <- chl_transect[with(chl_all, order(cruise, match(cast, cast_sort), match(niskin, niskin_sort),
                                    match(filter_size, size_sort))), ]
...
# to do: instead of being cruise-specific, use a standard list of stations L1-L13
# stace will add csv of stations (name, lat, lon)
# read csv into stations. declare ref_lon and ref_lat
...
add_station_info <- function(df){
  ...
  #   reduce(rbind)
# instead of this loop, apply loop over number of rows of full join table
