...
cruiselist <- c("en608","en617", "en627", "en644", "en649", "ar22", "ar32", "ar31a", "ar28a", "ar28b", "ar31b", "ar31c", "ar34a", "ar34b", "ar38", "ar39a", "ar39b", "ar61b", "en655", "en657", "en661", "en668", "en687", "at46")
#ar66b causes error
...
# add project_id
#chl_all$project_id <- ifelse(chl_all$cruise == "AR22" | chl_all$cruise == "AR32" | chl_all$cruise == "AR38", 
  #                                 yes = "JP", no = "LTER") #TODO: add OOI vs LTER distinction
...
chl_all <- chl_all  %>% 
    mutate(project_id_fancy  = case_when(cruise == "AR66B" & method_contributor == "method_Menden-Deuer" ~ "LTER",
                                   ...
                                   cruise == "AR38" ~ "JP",
                                   cruise == "AT46" ~ "LTER"))
# can get more granular using method contributor
chl_all <- chl_all  %>% 
...
qual_incorrect <- chl_all %>%
  filter(method_contributor == "method_Menden-Deuer") %>%
  filter(!is.na(quality_flag))

qual_iode_incorrect <- chl_all %>%
  filter(method_contributor == "method_Sosik" | method_contributor == "method_Rynearson") %>%
  filter(!is.na(iode_quality_flag))
...
#TODO: plot chl and phaeo by flag (boxplots)
