...
ar24b <- ar24b %>%
  mutate(
      niskin = ifelse(ra == 147.6, 
                      yes = "4-6",
                      no = ar24b$niskin)
      ) 
ar24b <- ar24b %>%
    mutate(
      niskin = ifelse(ra == 151.8, 
                      yes = "4-6",
                      no = ar24b$niskin)
...
ar24b <- ar24b %>%
      mutate(
      niskin = ifelse(ra == 108.7, 
                      yes = "9-10",
                      no = ar24b$niskin)
      )
ar24b <- ar24b %>%
    mutate(
      niskin = ifelse(ra == 102.5, 
                      yes = "9-10",
                      no = ar24b$niskin)
      )
...
Join with all other transect data

```{r}

#remove rows without OOI data
ar24 <- ar24all
...
ar24 <- ar24[, headers_ooi]


```

```{r}
# fill missing bottle metadata for those bottles joined from OOI Alfresco sheet
...
ar24temp <- full_join(ar24, cast7niskin6, by = c('cruise','cast','niskin'))
...
# columns not matching: bottle_other_method, filter_id_ooi, comments
# assemnbly rmd should read in this data set, then mutate bottle_other_method to NA
# assembly script should alter filter_id_ooi to alternate_sample_id (mutate, populate with NA for the other data)
# exclude comments

# write csv file 
write.csv(ar24, 'ar24.csv', row.names=FALSE)
```
